...
## 4-1. 進度數字與批次佇列的母體 (Denominator Consistency)
*   **同一支判定**：任何「還剩幾筆」的統計端點，母體必須與真正排進佇列的批次端點共用同一個函式（例：`app/routers/ingredient_mapping.py` 的 `_ingredient_pool()`），並吃相同的參數（如 `active_only`）。禁止在統計端點就地重寫篩選條件。
*   **兩個詞分開**：「尚未處理」＝佇列裡還沒跑到的（會歸零）；「未納入」＝依設定不會被排進佇列的（不是進度）。合成一個詞就會出現「卡片說還有一萬筆、批次說沒事可做」的自相矛盾畫面（v1.40.1 實際事故）。
*   **單位一致**：映射表唯一鍵若為 `(主鍵, 外部識別碼)`，同一實體可有多筆，統計一律用 distinct 主鍵，不要有的欄位數列、有的欄位數實體。
*   **回歸斷言**：測試中必須有一條「統計端點的 remaining == 批次端點會處理的筆數」。
