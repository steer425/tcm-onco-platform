...
但畫面語系預設繁體，使用者自然會輸入「當歸」。直接字串比對永遠比不到。
**失真跟靶點那次一樣是靜默的**：畫面不會說「比對失敗」，只會顯示「找不到」，
看起來就像資料庫沒收錄這味藥。

**而且這段邏輯先前被寫了兩次。** `js/pathways.js` 早在 v1.39.x 就處理好了，
其他六個搜尋框各自寫 `.toLowerCase().includes()`——於是通路富集分析頁查得到、
藥材關聯查詢站查不到。同一個平台兩種行為，跟靶點比對那次（八處）是同一族的錯。

**規則一：兩邊都折疊，不要「偵測輸入語系再轉換」。**

單向轉換只修好一半。藥材名稱在庫裡是簡體，但**疾病中文名稱是平台自己補的繁體**，
方向相反——把輸入轉成簡體反而讓繁體資料更找不到。而「偵測語系」本身也不可靠，
「人参」兩字簡繁同形，偵測不出來，也不需要偵測。
`zhNorm()` 把比對的兩邊都折到同一個標準形（小寫＋簡體），四種組合一次涵蓋。

**規則二：任何中文名稱的搜尋比對，一律呼叫 `frontend/js/zh-match.js`。**

```js
window.zhMatchAny([h.herb_cn_name, h.herb_pinyin, h.herb_en_name], keyword)
```

不要在頁面裡就地寫 `.toLowerCase().includes()`，也不要自己 `new OpenCC.Converter`。
頁面需要在 `js/i18n-dict.js` 之前載入 `<script src="js/zh-match.js"></script>`
（OpenCC 的 CDN script 之後）。

**規則三：OpenCC 載不到時要退化，不是壞掉。** OpenCC 由 CDN 提供，
`zhNorm()` 在拿不到時退回只做 lowercase／trim 的原字串比對——
搜尋回到修正前的行為，但查詢站不會整個打不開。`tests/test_zh_match.js` 有這條斷言。

**還沒涵蓋的：** `app/routers/gencc_diseases.py` 的關鍵字查詢是**伺服器端** `ilike`，
不吃這支。要修之前得先確認 GenCC 的中文名稱實際存的是繁是簡。
...
### 排名前幾條一定要點開通路圖看一眼（v1.39.8）

**統計告訴你「這條通路顯著」，只有通路圖能告訴你「這是不是一個機轉」。**

點開 KEGG 通路圖之後，看標色節點的**分佈形態**，兩種結果的意義完全不同：

| 形態 | 判讀 |
|---|---|
| 標色節點落在**同一條連續的訊號級聯**上（上游→下游） | 像是真的機轉 |
| 標色節點**散佈在彼此不相關的家族／分支**上 | 比較像資料來源的特性，不是協調的藥理作用 |

**實例（人參，2026-08-27 實測）：**

`hsa04080` Neuroactive ligand-receptor interaction 在統計上是全表最顯著的
（q = 3.33e-19、倍率 12.9）。但打開通路圖，標色的是
CHRM（乙醯膽鹼）、ADR（腎上腺素）、DRD（多巴胺）、HTR（血清素）、
OPR（類鴉片）、GABR、CHRN、NR3C1、PACAP——**散佈在彼此不相關的受體家族上，
沒有集中在任何一條連續級聯**。

這比較像「TCMSP 對人參皂苷做對接預測時所用的標靶 panel 涵蓋了很多種 GPCR」，
不太像單一的藥理作用。

對照組：凋亡那組命中 BAX／BCL2／CASP3／CASP8／CASP9，
是同一條粒線體級聯上的**連續節點**——那才是說得通的機轉。

**兩條通路的 q 值都極顯著，但一張圖就能分辨哪個是真機轉、哪個是資料來源的特性。
這件事統計數字永遠告訴不了你。**

規則：

- **排名前幾條（尤其倍率高得異常的）一律點開通路圖確認**再寫進任何結論
- 倍率越高越要看。異常高的倍率常常來自資料來源的取樣特性，不是生物學訊號
- 通路圖連結會把命中基因標色，就是為了讓這個檢查只需要幾秒鐘
- 判讀結果寫進報告時，要區分「機轉假說」與「靶點註記涵蓋範圍」——**兩者不是同一件事**

### 外部識別碼解析：只要有第二個欄位可以驗算，就一定要拿來驗（v1.40.0）

靶點標準化（F1-4）只能靠名稱比對，對不對只能靠人看。
成分標準化（F1-6）不一樣——**TCMSP 自己就存了分子量**，
所以每一筆解析都能拿它跟 PubChem 回傳的值對照。

**名稱對上就當作解析成功，是這類工作最容易犯、也最難發現的錯**：
畫面上一切正常、狀態顯示「自動採用」，只是那個 SMILES 屬於別的分子。
常見成因是同名異物、鹽類 vs 游離態、水合物。

規則：

- 分子量相符（±0.5 Da）→ 自動採用
- **分子量不符 → 一律進待確認**，並在註記寫出差多少與可能原因
  （差值接近 18 的倍數提示水合物、超過 20 Da 提示鹽類或同名異物）
- 任一邊缺值 → 可以自動採用但信心分數調降，註記標明「無法驗證」

**「無法驗證」跟「驗證通過」是兩回事**，所以驗算函式回傳三態
（True／False／None）而不是布林值。把缺值當成通過，等於讓驗證形同虛設。

覆蓋率統計要把「分子量不符而待確認」的筆數獨立列出來——
**那個數字本身就是驗證有沒有在運作的指標**，一直是 0 反而該懷疑。

### 名稱裡的後綴可能是化學意義，不是雜訊（2026-09-10，實際踩到）

**清理名稱之前，先確認每一個被拿掉的東西真的不帶意義。**
名稱正規化的目的是提高命中率，但拿掉一個有意義的記號，會讓查詢**成功地**
命中另一個化合物——比查不到危險得多。

實際事故：`app/tcmsp_pubchem.py` 的 `cleaned_name()` 有這一行

```python
n = re.sub(r"_qt$", "", n, flags=re.I)   # 註解寫「quantitative 後綴」——錯的
```

**TCMSP 的 `_qt` 是苷元（aglycone），指「把糖基拿掉之後的配基」。**
剝掉它之後查詢字串變成母體苷的名稱，於是 PubChem 回傳**帶著糖基的那個分子**，
分子量剛好多出糖基的整數倍（己糖 162.14、去氧己糖 146.14、戊糖 132.12）。

全庫實測（`python -m app.inspect_mw_mismatch`）：

- 待確認佇列 272 筆分子量不符裡，**81 筆是糖基差**
- 方向 **PubChem 較重 80 : TCMSP 較重 1**
- 那 81 筆裡 **75 筆（93%）** 名稱以 `_qt` 結尾

規則：

1. **差值能被化學解釋，不代表那筆可以採用——恰恰相反。**
   可解釋的是「為什麼錯」，不是「其實沒錯」。糖基差要**否決重解**，不是確認
2. 分子量不符的提示要涵蓋糖苷（目前 `mw_check` 只提鹽類、水合物、同名異物，
   照那個提示看不出真正的成因）
3. 來源名稱帶化學意義的後綴時，**不要靜默剝除**；剝除後的命中要另外標記，
   不要跟一般待確認混在同一個佇列裡等人工逐筆看
4. `resolve_name()` 在名稱命中多筆時直接回待確認，**沒有用到手上的 `tcmsp_mw`**。
   候選裡若只有一筆分子量吻合，那一筆幾乎確定就是答案

⚠️ **風險**：那 81 筆現在都帶著 CID／SMILES／InChIKey，全部屬於錯的分子。
照「看起來像就按確認」清佇列，會讓 81 個錯誤結構進入資料庫，
而且之後 Step 6 拿 InChIKey 去對 DepMap／GDSC **不會有任何地方報錯**，
只是對到別的化合物。

### 分類統計要分層，合併算會被最大的那一桶帶偏（2026-09-10）

同一份資料，全庫合併算方向得到「TCMSP 較重 1252 : PubChem 較重 266，假設不成立」，
分層之後得到「糖基桶 80 : 1，假設成立」。**前者是錯的**：1,575 筆裡有 1,311 筆是
「分子量相符」，那一桶的方向純粹是兩邊小數位慣例的差別（TCMSP 系統性偏高），
是雜訊，卻因為佔比最大而主導了結論。

同一次分析還把早就自動採用的 1,303 筆算進「可以整批處理」，
灌出一個很大但沒有任何待辦意義的數字。

規則：

- **檢定某個子群的假設，就只在那個子群裡算**，不要全體合併
- 任何「還有多少可以處理」的數字，**先按 status 切掉已經處理完的**
- 事後補分類單位（醯基、醣醛酸之類）要**單獨報告、明講不作為結論依據**：
  可用的小質量單位一多，任何差值都湊得出來，那時分類就不再是證據而是事後配適

這與第五之一章「顯示篩選不得改動 n 與 N」、上面「進度數字與批次佇列必須共用母體」
是同一族錯誤：**統計方法沒錯、數字都在合理範圍、畫面很合理，但母體選錯，
回答的就是別的問題。**

### 驗收腳本指定藥材一律用 herb_id，不要用中文名（2026-09-10）

TCMSP 匯入的藥材名稱是**簡體**——用繁體「人參」查，一筆都不會中，
而畫面只會說「找不到」，看起來像資料沒匯進來。

而且「人参」（herb_id 336，目標一的驗證對象）與「人参叶」（337）只差一個字，
「ginseng」還會同時命中三七（346／347）。**靠名稱比對很容易默默驗錯對象。**

規則：驗收與分析腳本一律以 `herb_id` 為參數；名稱只作為查 id 的輔助，
且命中多筆時**列出來讓人指定，不要自己挑一個**。

### 富集分析要對照**可比的基因集合**，不只對照母體（v1.41.6）

超幾何檢定的虛無假設是「命中的成員隨機分佈」。當研究集合本身有系統性偏差時，
那個虛無假設太鬆——**正確的對照不是隨機成員，是可比的基因集合**。

實際事故：DepMap lineage 富集顯示人參的選擇性靶點在 Lymphoid 顯著富集
（q=2.7e-6）。但淋巴系細胞株本來就依賴較多，所以**任何**一組同樣大小的
選擇性基因可能都會得到類似結果。

以刻意造的 fixture 驗證過：淋巴系依賴率 6 倍、藥材靶點隨機挑，
超幾何給出 q=6.95e-08「顯著」（偽陽性），對照基因集給出經驗 p=0.771（正確）。

規則：

1. 任何「這個藥材集中在 X」的結論，都要附一個**同樣大小、同樣通過前置篩選**
   的隨機基因集對照，報告經驗 p 值
2. 對照池的篩選條件必須與研究集合**一致**（同樣非泛必需、同樣通過選擇性門檻），
   否則比較的是篩選條件不是生物學
3. 亂數種子要固定並寫進輸出，結果必須可重現
4. 「落在對照範圍內」要明文寫成**不是該藥材的特性**，不要只寫「未達顯著」

這與既有的「拿一組全部都是藥物標靶的基因對比絕大多數不是藥物標靶的母體，
結論必然顯著」是同一個病根的第三種變形。

### 富集分析的背景，不能含「不可能進入研究集合」的成員（v1.41.5）

既有規範寫的是「背景基因數必須來自外部資料庫，拿我方靶點當母體是循環論證」。
這是同一個病根的另一種變形，方向相反但一樣致命。

實際事故：DepMap 的 lineage 富集用 `Model.csv` 全體 2,154 株當背景，
但只有 1,208 株做過 CRISPR 篩選。那 900 多株**不可能出現在命中集合裡**，
而且**各 lineage 的篩選覆蓋率不同**——於是測到的是篩選覆蓋率，不是生物學。
報表因此宣稱膀胱／泌尿道富集 q=1.7e-3 顯著；換成正確母體後倍率從 2.13 掉到約 1.19。

規則：

1. 背景必須是**有機會進入研究集合的全部成員**，一個不多一個不少。
   「資料庫收錄」與「這次實驗測過」是兩件事
2. 報表要**同時列出母體數與它的定義**，讓母體的選擇看得見、可被質疑
3. 加**自檢**：母體數與另一條獨立途徑得到的數字（例如摘要表記錄的樣本數）
   差距超過門檻就印警告，不要靜默通過

### 外部資料庫的基因清單，一律用 `standardized_symbols()`（v1.41.0）

`app/target_index.py` 的比對有兩層，第二層是「把靶點名稱拆成英數字詞」。
那是 UniProt 標準化跑完之前刻意保留的站內退路，**但不可以拿去跟外部基因清單取交集**。

實測：靶點名稱「Histone chaperone SET and MAX interacting protein」會被拆出
**SET** 與 **MAX**——兩個都是真實的 HGNC 基因符號。拿這組字詞去挑 DepMap 的基因欄位，
那兩個基因就會被當成 TCMSP 靶點匯入，而且畫面上看起來完全正常。

規則：凡是要跟外部資料庫的基因清單對接，用 `standardized_symbols()`（只回第一層）；
站內統計與查詢站維持用 `symbol_to_targets()`（含退路）。**兩者不可互換。**

### DepMap 是稀疏儲存，判讀時門檻要跟數字一起看（v1.41.0）

`depmap_gene_dependency` **不是完整矩陣**，只有 gene effect 達門檻的配對
（預設 ≤ −0.5，實際值記在 `depmap_import_runs.effect_threshold`）。

- 「某基因只有 N 筆」代表**達門檻的株數**，不是「只在 N 株測過」。
  母體一律看 `depmap_gene_summary.n_lines_total`
- 換了門檻，`n_dependent` 的意義就變了，**兩次匯入的數字不能直接比較**
- 缺值（`NA`／空字串）不計入母體，也不當成 0——當成 0 會讓「資料缺值」
  偽裝成「不依賴」。這與 `pathways._num()` 對 ADME 缺值的處理是同一個原則

**泛必需基因（common essential）必須排除在抗腫瘤排序之外。**
命中泛必需基因不是機轉，是細胞毒性——任何藥物殺得死細胞都會「命中」它們。
旗標在 `depmap_gene_summary.is_common_essential`。

選擇性的定義是「達門檻株數佔母體的比例低，**而且不是泛必需**」，
兩個條件缺一不可。

### 外部資料的授權要在動工前查，不是上線前（v1.41.0）

實際情況：DepMap 是 CC BY 4.0（可商用、可再散布，**須標註出處**），
但 GDSC 的 Sanger 使用政策禁止「單獨或與其他資料組合後轉售，或用於提供商業服務」——
與 AlphaGenome 同一類問題。兩者長得很像，條款卻完全不同。

規則：接任何外部資料源之前先查條款並寫進 `CHANGELOG`；
可商用與否會決定這個模組能不能對外，比寫程式更早需要答案。
**Zenodo 之類的第三方鏡像掛什麼授權不算數**，要看原始提供者的政策。

顯示 CC BY 資料的前端頁面必須標註來源與釋出版本。

### 全庫層級的數字回答不了逐一對象的驗收條件（v1.42.0）

F1-6 的覆蓋率是**全庫**數字（1,303／2,632 = 49.5%）。目標一的驗收條件卻是
「人參那 22 個活性成分，是不是**每一個**都拿到了結構」——**全庫 49.5% 不等於這 22 筆都到手**，
可能全中、也可能一筆都沒中，兩者在全庫卡片上長得一模一樣。

**準則：只要驗收條件是「某個特定對象的每一筆」，就需要一個逐筆列出的畫面，
不能拿全庫百分比當證據。** 這類頁面（v1.42.0 起為 F1-7）與全庫批次頁並存，
兩者回答不同問題，不要合併。

實作時三件事不能省：

1. **母體判定共用同一支函式**，不在端點裡重寫（本章上一節的規矩）
2. **「尚未處理」與「查過了但查無結果」要用不同狀態值**（`untouched` vs `unresolved`）。
   混在一起會讓「還沒跑」看起來像「跑過但找不到」
3. **測試要直接比對共用函式的輸出**，不要只斷言數字剛好對。
   斷言「端點的活性成分清單 == `pathways.active_ingredients()` 的結果」，
   日後任何人在端點裡重寫篩選條件，測試就會紅

### 資料庫沒有的欄位，不要在端點或前端臨時湊（v1.42.0）

TCMSP 的成分只有英文原名，**沒有中文名稱、也沒有化學分類欄位**。
交付報告時可以另外整理一份中英對照供人閱讀，但那是編輯產出、有整理者也有出處；
**平台端點與頁面不得提供這種查不到出處的對照**——一旦放進系統就會被當成資料庫欄位引用，
而且沒有人維護。要做就正式建一張對照表並定義來源與維護流程，那是獨立的一件事。

### 進度數字與批次佇列必須共用同一支母體判定（v1.40.1，實際踩過兩次）

**任何「還剩幾筆」的畫面數字，都必須跟真正排進佇列的那支查詢用同一個母體。**
不同意就不要顯示，寧可少一個數字。

實際事故：成分標準化（F1-6）的覆蓋率卡片數全表 13,728 筆，批次佇列另外套了
「名稱非空 ＋ OB／DL 達標」兩層篩選（母體 2,632）。結果卡片顯示「尚未處理 11,096」，
而批次連按十三次都回「這一批沒有需要處理的成分了」——**程式跑得動、數字都在合理
範圍、畫面也很合理，但兩個數字回答的是不同的問題**。

做法（`app/routers/ingredient_mapping.py` 的 `_ingredient_pool()`）：

1. 母體判定寫成**一支函式**，統計端點與批次端點都呼叫它，不准任何一邊就地重寫。
   跟第五之一章「靶點比對只有一份」是同一個原則
2. 統計端點要吃**跟批次一樣的參數**（例如 `active_only`），前端的勾選同時決定兩邊
3. **「尚未處理」與「未納入」必須是兩個不同的詞**：
   前者是佇列裡還沒跑到的（會歸零，是進度），後者是照設定就不會被排進去的（不是進度）。
   同一個詞蓋住兩件事，就是誤判的直接原因
4. 「未納入」要附**原因細分**（未達標／缺值／資料不全）。合成一個數字看不出
   「有多少筆是資料本身有問題」，那通常比篩選結果更值得處理
5. 進度條吃母體，不吃全庫。吃全庫會永遠停在個位數，看起來像卡住
6. 「全部處理完畢」一定要講範圍：「**活性成分**已全部處理完畢（另有 N 筆未納入）」
7. 加一條回歸斷言：**統計端點的 `remaining` 等於批次端點實際會處理的筆數**。
   有這條，未來任何一邊改了篩選而另一邊沒跟上，測試就會擋下來

另外，同一張卡片上的數字**單位要一致**。映射表的唯一鍵若是 `(主鍵, 外部識別碼)`，
同一個實體合法可以有多筆，這時「筆數」與「不重複實體數」會分岔——
一律用不重複實體數（distinct），不要有的欄位數列、有的欄位數實體。

### 化合物名稱清理不要動立體化學（v1.40.0）

`cleaned_name()` 可以剝掉 TCMSP 的 `_qt` 後綴、方括號與括號註記，
但**必須保留 `beta-`、`alpha-`、`(2S)-`、`(+)-`、`(-)-` 這類前綴**。

`beta-sitosterol` 與 `sitosterol` 不是同一個化合物，剝掉會解析到錯的東西，
而且分子量往往相同——**這種錯連分子量驗證都攔不下來，只能靠人看**。
所以原名查不到、清理後才查得到的，即使分子量相符也一律進待確認。
...
### CI／排程設定檔絕對不要放進 `.gitignore`（v1.40.1，實際踩過）

`.gitignore` 曾有一行 `.github/workflows/`，導致 `daily-news.yml` 在本機存在、
卻**從來沒有進過版控**。GitHub Actions 只執行 repo 裡實際存在的 workflow，
所以那條排程自建立起就沒有跑過——**而且沒有任何錯誤訊息**，
本機看得到檔案、`git status` 也乾淨，一切都很正常的樣子。

判斷準則：**「這個檔案要不要在伺服器上生效」**。要的話它就必須進版控，
不管它是不是 `.` 開頭、是不是設定檔。`.gitignore` 只該擋
「純本機、每台機器不同、或不該外流」的東西（`.env`、`*.db`、`__pycache__`、
`.claude/settings.local.json`）。

驗證方式不是看本機有沒有這個檔案，而是：

```powershell
git ls-files .github          # 沒有輸出就代表 GitHub 上沒有
git check-ignore -v <路徑>    # 會指出是 .gitignore 第幾行擋掉的
```

同一個檢查也適用 `render.yaml`、`requirements.txt` 這類部署設定。
...
| `ingredient-mapping.html` | F1-6 | 成分標準化（PubChem）：覆蓋率、分批解析、人工審核（含結構圖與分子量對照）、以 InChIKey／CAS／CID 反查 |
...
### 版本號要同步的六個地方（v1.42.0 更正：`git_push.bat` 裡有兩處）
...
| `git_push.bat` | **`EXPECTED_VER`** —— pre-flight 用它去比對上面四個檔案 |
| `git_push.bat` | `DEFAULT_MSG`（不改的話 commit 訊息會沿用上一版，git log 會對不上） |

⚠️ **`git_push.bat` 有兩行要改，不是一行。** 該檔第 34 行的註解寫著
「update these two lines every release」。v1.42.0 交付時只改了 `DEFAULT_MSG`，
pre-flight 因此以舊版號去比對四個檔案、四項全 FAIL——**擋下來是對的，那正是它的用途**。
...
- `python -m tests.test_news_e2e`、`python -m tests.test_target_mapping`、
  `python -m tests.test_pathways` 與 `python -m tests.test_ingredient_mapping` 全過（**不是** `python -m pytest tests/ -q`——`test_news_e2e.py`
