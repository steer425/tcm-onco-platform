...
# 這些功能代碼一定會顯示給任何登入中的有效帳號，不需要額外指派角色/權限矩陣。
# 目的：避免帳號被啟用後、但還沒被指派任何角色時，完全看不到任何畫面（一片空白）。
# Dashboard 是全站的預設落地頁，理當是「只要登入就看得到」的基礎保底項目。
BASELINE_CODES = {"F0-13", "F0-13-1", "F0-13-2", "F0-13-3", "F0-13-4", "F0-13-5", "F0-18"}
...
def get_menu(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if is_admin(current_user):
        ...
        visible = [f for f in features if f.id in allowed_ids or f.code in BASELINE_CODES]
